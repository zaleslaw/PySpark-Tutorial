...
if __name__ == "__main__":
    ...
    output.cache()
    layers = [16, 50, 100, 16, 2]  # first value is equal size of input vector (tensor), the final value is equal amount
    # of classes, should be more than 1

    # Step - 3: Set up the LinearSVC Classifier
    trainer = MultilayerPerceptronClassifier(labelCol="eatable", featuresCol="features", maxIter=200, seed=1234,
                                             layers=layers,
                                             blockSize=10,
                                             stepSize=0.001)
