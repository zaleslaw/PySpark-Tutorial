from pyspark.ml.classification import DecisionTreeClassifier, LinearSVC, OneVsRest, LogisticRegression
...
if __name__ == "__main__":
    ...
    trainer2 = LogisticRegression(featuresCol="features", regParam=0.4, labelCol="type") # tune reg param # change classification algorithm

    multiClassTrainer = OneVsRest(labelCol="type", classifier=trainer2)

    # Step - 4: Train the model
    model = multiClassTrainer.fit(output)

    for m in model.models:
        print(str(m.coefficients) + " " + str(m.intercept))
